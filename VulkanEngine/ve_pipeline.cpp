...
namespace ve {
	...
	VePipeline::VePipeline(VeDevice& device, const PipelineConfigInfo& configInfo, const std::string& vertFilepath, const std::string& fragFilepath) : veDevice{ device } {
	...
	std::vector<char> VePipeline::readFile(const std::string& filepath) {
		...
		for (const char& c : filepath) {
			if (!isprint(c)) {
				std::cout << "Non-printable character found in filepath at position: "
					<< &c - filepath.data() << std::endl;
			}
		}
	...
	void VePipeline::createGraphicsPipeline(const PipelineConfigInfo& configInfo, const std::string& vertFilepath, const std::string& fragFilepath) {
		...
		shaderStages[0].pName = "main"; //name of main function (entry function) of vertex shader, need to match actual function name in shader
		...
		shaderStages[1].pName = "main"; //name of main function (entry function) of vertex shader, need to match actual function name in shader
