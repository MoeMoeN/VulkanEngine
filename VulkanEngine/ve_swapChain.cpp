...
namespace ve {
    ...
    VeSwapChain::VeSwapChain(VeDevice& deviceRef, VkExtent2D extent, std::shared_ptr<VeSwapChain> previous)
        ...
        //cleanup up old swap chgain since it's no longer needed
    ...
    VkPresentModeKHR VeSwapChain::chooseSwapPresentMode(
        ...
        for (const auto& availablePresentMode : availablePresentModes) {
            if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) { //high power consuption (not recommended on mobile)
        ...
        // for (const auto &availablePresentMode : availablePresentModes) {
        //   if (availablePresentMode == VK_PRESENT_MODE_IMMEDIATE_KHR) { //low latency (high performance), but visible tearing in high power consumption
        ...
        return VK_PRESENT_MODE_FIFO_KHR; //this is only present mode that is guaranteed to be avaible. every different mode need to be checked first if it's avaible on choosen device (V-SYNC)
    }
